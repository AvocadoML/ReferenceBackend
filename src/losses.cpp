...
namespace
{
	...
	template<typename T>
	void gradient_helper(avLossType_t lossType, T *gradient, const T *output, const T *target, av_int64 elements, T alpha, T beta,
			bool fused) noexcept
	{
		switch (lossType)
		{
...
namespace avocado
{
	namespace backend
	{
		...
		{
			const av_int64 elements = getTensor(outputDesc).volume();
